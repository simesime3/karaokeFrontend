...
export default function Home() {
  ...
  const handleSearch = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch(`https://tech0-gen-8-step3-testapp-py1-11.azurewebsites.net/search?q=${query}`);
  ...
  const registerPerformance = async (songId, title, artist) => {
    ...
    try {
      const res = await fetch('https://tech0-gen-8-step3-testapp-py1-11.azurewebsites.net/register-performance', {
        method: 'POST',
        headers: {
  ...
  useEffect(() => {
    ...
    const fetchPeople = async () => {
      const res = await fetch('https://tech0-gen-8-step3-testapp-py1-11.azurewebsites.net/people');
      const data = await res.json();
      setPeople(data);
    };
  ...
  const fetchPerformances = async (personId) => {
    const res = await fetch(`https://tech0-gen-8-step3-testapp-py1-11.azurewebsites.net/performances/${personId}`);
    const data = await res.json();
